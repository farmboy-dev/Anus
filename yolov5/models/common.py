...
class AutoShape(nn.Module):
    ...
    # save_dir = None # for saving images from the crawler
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        for i, im in enumerate(imgs):
            ...
            if isinstance(im, (str, Path)):  # filename or uri
                ...
                # print("hash value:", img_hash, type(img_hash))
                # if hash value is in database -> rejected
                o_im = im.copy()                # --
        ...
        with amp.autocast(autocast):
            ...
            return Detections(imgs, y, files, o_im, img_hash, t, self.names, x.shape)


class Detections:
    # YOLOv5 detections class for inference results
    def __init__(self, imgs, pred, files, original_img, hash_img, times=(0, 0, 0, 0), names=None, shape=None):
        super().__init__()
        ...
        self.o_im = original_img # original image
        self.hash_img = hash_img # image hash value
        self.cropped_filelist = []
        self.image_size = []
    ...
    def display(self, pprint=False, show=False, save=False, crop=False, render=False, labels=True, save_dir=Path('')):
        ...
        for i, (im, pred) in enumerate(zip(self.imgs, self.pred)):
            ...
            if pred.shape[0]:
                #added part
                self.o_im.save('./collected_images/images/'+str(self.hash_img)+'.jpg')
                self.image_size.append(self.o_im.size)
                # print(self.pandas().xywhn[0].values)
                temp_cropped_info = []
                for value in self.pandas().xywhn[0].values:
